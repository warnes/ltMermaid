...
.PHONY: all build test l3build-check l3build-luatex-check l3build-test l3build-clean install local-install clean
...
local-install:
	@if [ -z "$(LOCAL_MKTEX)" ]; then \
		echo "Error: LOCAL_MKTEX must be set, e.g. make local-install LOCAL_MKTEX=/path/to/texmf"; exit 1; \
	fi
