...
\item \textbf{\cmd{mermaidfile}\texttt{\{filename\}}:}
  Render a Mermaid diagram from an external file (\texttt{.mmd}) instead of inline code. The diagram is rendered and included just like the \texttt{mermaid} environment, but the source is read from the specified file.
\end{itemize}
\par}

\subsection{Including diagrams from files}

You can use \cmd{mermaidfile}\texttt{\{filename\}} to render a Mermaid diagram from an external file. Example:
